...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    model.add(Dense(dataset.get_num_labels(), activation='softmax'))    # à verifier
    ...
    os.makedirs(artefacts_path, exist_ok=True)  #exist_ok=True to avoid error if the folder already exists
    ...
    with open(os.path.join(artefacts_path, 'params.json'), 'w') as f:   #w for write
        json.dump(train_conf, f)    #dump the train_conf in the file f
