...
class BaseTextCategorizationDataset:
    ...
    def _get_num_train_samples(self):
        ...
        (training set size)
        """
        return integer_floor(self.train_ratio * self._get_num_samples())    #we used integer_floor to round down the number and get an integer
    ...
    def _get_num_train_batches(self):
        ...
        """
        return integer_floor(self._get_num_train_samples() / self.batch_size)   #we used integer_floor to round down the number and get an integer

    def _get_num_test_batches(self):
        """
        returns number of test batches
        """
        return integer_floor(self._get_num_test_samples() / self.batch_size)    #we used integer_floor to round down the number and get an integer
    ...
    def get_label_to_index_map(self):
        ...
        """
        return {label: index for index, label in self.get_index_to_label_map().items()}  #we used items() to get the key and value of the dictionary
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def __init__(self, filename, batch_size,
        ...
        self._label_list = list(self._dataset['tag_name'].unique())  #we used unique() to get the unique values of the column tag_name, we could also use a set but it's less eficient than unique()
        #self._label_list = list(set(self._dataset['tag_name]'].value))             here it's the same but with a set
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                """
                df_tag_filter = df[df['tag_position'] == position]
        ...
        pre_processed_dataset = dataset.pipe(filter_tag_position(0)).pipe(filter_tags_with_less_than_x_samples(min_samples_per_label))
    ...
    def get_train_batch(self):
        ...
        # When we reach the max num batches, we start a new one 
